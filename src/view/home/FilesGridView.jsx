...
export const FilesGridView = {
    view(vnode) {
        ...
        if (metadatas.length === 0) return m("h2.empty", "Empty");
        ...
        return metadatas.map((metadata, i) => {
            ...
            if (sysdata.is_hidden && !vnode.attrs.showHidden)
                return m(".hidden", { key: i });
            // TODO Find new integration for poper js this throws a warning
            return m(Tooltip, {
                key: i,
                hoverOpenDelay: 1000,
                ...
                trigger: m(
                    "div",
                    {
                        key: i,
                        class: `file${
                            vnode.attrs.isSelected(i) ? " selected" : ""
                        }`,
                        onclick() {
                            vnode.attrs.onSelect(i);
                        },
                        ondblclick() {
                            vnode.attrs.open(unwrap(metadata));
                        },
                    },
